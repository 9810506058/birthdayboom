...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numbersOnly.length > 4) {
      formattedDate =
        numbersOnly.slice(0, 4) + '/' + numbersOnly.slice(4);
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedDate) {
      setErrorMessage('Please enter our special date first 💕');
      return;
    }

    // Convert 2061/05/09 → 2061-05-09
    const normalizedDate = selectedDate.replace(/\//g, '-');

    // Compare with SITE_CONFIG.specialDate
    if (normalizedDate === SITE_CONFIG.specialDate) {
      setIsSuccess(true);
      setErrorMessage('');

      // Remember that the website has been unlocked
      ...
      // Give the success animation some time
      setTimeout(() => {
        onLoginSuccess();
      }, 2200);
    } else {
      ...
      const randomMsg =
        PLAYFUL_ERROR_MESSAGES[
          errorCount % PLAYFUL_ERROR_MESSAGES.length
        ];
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-left space-y-1.5">
              ...
              >
                ...
                <span>
                  Enter the date that means the most to us 💕
                </span>
              </label>
              ...
              <div className="relative">
                <input
                  ...
                  maxLength={10}
                  className="w-full px-4 py-3.5 rounded-2xl bg-rose-50/60 dark:bg-slate-800/80 border border-rose-200 dark:border-rose-800/80 text-slate-800 dark:text-slate-100 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-rose-400 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
                />
              </div>

              <p className="text-[11px] text-slate-400 dark:text-slate-500 px-1">
                Enter your special date like 2061/05/09 💕
              </p>
            </div>
